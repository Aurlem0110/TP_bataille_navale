...
# Définition d'un chemin absolu vers le dossier des sons
BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # Répertoire de bataille_na.py
...
def test_sons():
    """
    Test de tous les fichiers sons dans le dossier sounds.
    """
...
class Navire:
    def __init__(self, nom, taille):
        """
        Initialisation du navire avec un nom, une taille, et sa position.
        """
        self.nom = nom
        self.taille = taille
        self.positions = []  # Position du navire
        self.touchees = []
        self.est_coule_flag = False  # État du navire

    def ajouter_positions(self, positions):
        """
        Définition des positions du navire.
        """
    ...
    def est_touche(self, position):
        """
        Marquage d'une position touchée.
        """
        if position in self.positions and position not in self.touchees:
            self.touchees.append(position)
            print(f"[LOG] navire {self.nom} est touché : {position}")
        else:
            print(f"[LOG] navire {self.nom} est déjà touché ou coup tombé à l'eau : {position}")

    def verifier_etat(self):
        """
        Vérification si le navire est complètement coulé.
        """
        if all(position in self.touchees for position in self.positions):
            self.est_coule_flag = True
            print(f"[LOG] navire {self.nom} a coulé!")
            return True
        return False
...
class Plateau:
    def __init__(self, taille=10):
        """
        Initialisation du plateau sur une grille.
        """
        self.taille = taille
        self.grille = [[None for _ in range(taille)] for _ in range(taille)]
        self.navires = []  # navires situés sur le plateauu

        def est_position_valide(self, positions):
        """
        Vérification de la validité des positions dans la grille
        """
    ...
    def tirer(self, position):
        """
        Tentative d'un tir sur une position
        """
        x, y = position
        ...
        if self.grille[x][y] is None:
            print(f"[LOG] tir à la position {position} manquée.")
            return "manqué"
        else:
            ...
            if navire.verifier_etat():
                ...
                return "coulé"
            print(f"[LOG] le tir à la position {position} a touché le navire {navire.nom}.")
            return "touché"
...
class Joueur:
    def __init__(self, nom, est_humain=True):
        """
        Initialisation du joueur (humain ou ordinateur).
        """
        self.nom = nom
    ...
    def tirer(self, adversaire, position):
        """
        Tentative de tir sur le plateau de l'adversaire
        """
    ...
    def tirer_sur_ordinateur(self, x, y):
        """
        Gestion du tir sur le plateau de l'ordinateur
        """
        position = (x, y)

        if self.ordinateur.plateau.grille[x][y] is None:
            print(f"[LOG] le tir à la position {position} est manqué")
            ...
            if navire.verifier_etat():
                ...
                print(f"[LOG] le navire {navire.nom} coulé.")
    ...
    def tir_ordinateur(self):
        """
        L'ordinateur tire sur le plateau du joueur
        """
        while True:
    ...
    def disable_grids(self):
        """
        Désactivation des grilles après la fin de la partie.
        """
...
# Génération des positions aléatoires pour les navires
def generer_positions_aleatoires(taille, plateau):
    """
    Génèration d'une liste de positions valides aléatoires pour un navire
    """
    while True:
        orientation = random.choice(["horizontal", "vertical"])
        if orientation == "horizontal":
        ...
        # Vérification de la validité & de l'accessibilité de toutes les positions
...
class BatailleNavaleApp:
    def __init__(self, master):
        ...
        self.joueur1_bateaux_restants = len(self.navires)  # nombre de navires du J1
    ...
    def afficher_menu_accueil(self):
        """
        Affichage d'un menu d'accueil avec les règles du jeu
        """
        ...
        regles = (
            ...
            "- L'objectif est de trouver les navires de l'adversaire.\n"
            "- Un tir touché est marqué en rouge, un tir tombé à l'eau en bleu.\n"
            "- Le premier à couler tous les navires de son adversaire gagne.\n\n"
        ...
        self.difficulte.set("Facile")# Valeur mise par défaut
    ...
    def lancer_jeu(self):
        """
        Fermeture du menu d'accueil et affichage de la grille du jeu
        """
    ...
    def previsualiser_navire(self, x, y):
        """
        Coloriage des cases pour prévisualiser le placement du navire
        """
    ...
    def reinitialiser_previsualisation(self, x, y):
        """
        Réinitialisation des couleurs des cases après le survol, sauf pour les cases validées
        """
        if self.navire_actuel_index >= len(self.navires):  # Vérification
            return
        ...
        if len(positions) == navire.taille:
            for px, py in positions:
                if self.grille_buttons[px][py].cget("bg") == "lightgreen":  # Réinitialiser uniquement les cases en prévisualisation
                    self.grille_buttons[px][py].config(bg="#ADD8E6")
    ...
    def selectionner_case(self, x, y):
        ...
        if self.navire_actuel_index >= len(self.navires):
            ...
            # Suppression des boutons et labels liés à l'orientation
    ...
    def valider_placement(self):
        """
        Validation du placement d'un navire
        """
        navire = self.navires[self.navire_actuel_index]
    ...
    def placer_navires_ordinateur(self):
        """
        Placement des navires de l'ordinateur sans chevauchement et uniquement des navires autorisés
        """
    ...
    def tirer_sur_ordinateur(self, x, y):
        """
        Gestion d'un tir sur le plateau de l'ordinateur
        """
    ...
    def tir_ordinateur(self):
        """
        L'ordinateur effectue un tir sur le plateau du joueur, en fonction de la difficulté
        """
        self.nombre_tours += 0.5  # Ajout de 0.5 pour compter un tour complet après le tir du joueur.
    ...
    def changer_orientation(self):
        """
        Changemenet de l'orientation entre horizontal et vertical
    ...
    def disable_player_grid(self):
        """
        Désactivation de la grille du joueur après la fin de la partie
        """
        for row in self.grille_buttons:
            for btn in row:
                btn.config(state="disabled")
    ...
    def verifier_fin_de_jeu(self):
        """
        Vérification si la partie est terminée et affichage de l'écran de fin si nécessaire
        """
    ...
    def afficher_message_fin(self, message):
        """
        Affichage d'un message de fin de partie dans une fenêtre popup
        """
        import tkinter as tk
        from tkinter import messagebox
    ...
    def reinitialiser_jeu(self):
        """
        Réinitialisation de la partie en recréant les grilles et les navires
        """
    ...
    def creer_grilles(self):
        """
        Création ou réinitialisation des grilles du joueur et de l'ordinateur
        """
    ...
    def rafraichir_interface(self):
        """
        Mise à jour de l'interface graphique après une réinitialisation
        """
        ...
        print("[LOG] interface graphique mise à jour.")

    def creer_interface_grille(self, frame, joueur, est_joueur):
        """
        Création de l'interface graphique pour une grille
        """
        for i in range(10):
            for j in range(10):
    ...
    def afficher_ecran_fin(self, message, statistiques=None):
        """
        Affichage à l'écran de fin avec un message et les statistiques
        """
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = BatailleNavaleApp(root)# Nouvelle instance
    app.afficher_menu_accueil() # Affichage du menu d'accueil
    root.mainloop()